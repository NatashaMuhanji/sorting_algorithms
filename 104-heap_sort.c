 ...
 *
**/
void swift_check(int *array, size_t size_init, size_t size, size_t i)
{
	...
	if (br1 < size - 1)
		swift_check(array, size_init, size, br1);
	if (br2 < size - 1)
		swift_check(array, size_init, size, br2);
}
/**
...
void heap_sort(int *array, size_t size)
{
	...
	for (i = 0; i < size / 2 ; i++)
	{
		swift_check(array, size_init, size, size / 2 - 1 - i);
	}
	for (i = 0; i < size_init - 1; i++)
	{
		...
		array[size - 1 - i] = n;
		print_array(array, size_init);
		swift_check(array, size_init, size - i - 1, 0);
	}
